...
export default function EditRewardPage() {
    ...
    const fetchReward = async () => {
        try {
            const res = await fetch(`/api/rewards/${params.id}`);
            const data = await res.json();
            if (data.success) {
                setFormData({
                    title: data.reward.title || '',
                    description: data.reward.description || '',
                    cost: data.reward.cost || 10,
                    image: data.reward.image || '',
                    category: data.reward.category || 'other',
                    stock: data.reward.stock ?? -1
                });
            }
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (data.success) {
                ...
                setError(data.error || "Sovg'ani yangilashda xatolik");
            }
        } catch (err) {
            setError("Sovg'ani yangilashda xatolik");
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div style={{ maxWidth: '600px' }}>
            <div className="d-flex align-items-center gap-3 mb-4">
                ...
                <h1 className="h3 fw-bold mb-0">Sovg'ani tahrirlash</h1>
            </div>
