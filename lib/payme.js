 ...
 * @param {number} amount - Amount in tiyin (1 so'm = 100 tiyin)
...
export function generatePaymeUrl(orderId, amount, account) {
    ...
    const params = {
        ...
        ac: account,
        a: amountInTiyin,
        c: PAYME_CONFIG.callbackUrl,
        ct: 3600000, // Timeout 1 hour
    };
...
export class PaymeRPC {
    ...
    async checkPerformTransaction(params) {
        ...
        const user = await this.db.collection('users').findOne({ 
            _id: new this.db.ObjectId(account.userId) 
        });
    ...
    async performTransaction(params) {
        ...
        if (transaction.state === 1) {
            ...
            await this.db.collection('users').updateOne(
                { _id: new this.db.ObjectId(transaction.userId) },
